...
public class MainActivity extends AppCompatActivity { 
    ...
    protected void onCreate(Bundle savedInstanceState) { 
        ...
        dbHandler = new MyDBHandler(getApplicationContext());  db = dbHandler.getWritableDatabase(); 
    } 
    public void printDatabaseData(View V){ 
        String query = "SELECT * FROM " + MyDBHandler.TABLE_NAME; 
        String dbData = " ";  
        Cursor c = db.rawQuery(query, null);   c.moveToFirst(); 
        while (!c.isAfterLast()) { 
            ...
            dbData += "\n"; 
            c.moveToNext();  } 
        c.close(); Toast.makeText(getApplicationContext(),dbData,Toast.LENGTH_LON G).show(); 
    } 
    public void insForm(View V){ 
        Intent t = new Intent(this,insTsk.class);  startActivity(t); 
        dbHandler.close(); 
        finish(); 
    } 
    public void delForm(View V){ 
        Intent t = new Intent(this,delTsk.class);  startActivity(t); 
        dbHandler.close(); 
        finish();
    }  }